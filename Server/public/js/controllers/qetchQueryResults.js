...
QetchQuery.controller('QetchQuery_ResultsCntrl',
    ['$scope', '$timeout', 'QetchQuery_QueryAPI', 'Dataset_Resource', 'DatasetAPI', 'Parameters',
        function ($scope, $timeout, QetchQuery_QueryAPI, Dataset_Resource, DatasetAPI, Parameters) {
            ...
            $scope.customersRatio = function (matchValue) {
                let sectionsCustomers = matchValue.points.map(e => JSON.parse(e.metadata.customers)).concat();
                let customers = JSON.parse("[" + $("#customers").val() + "]")
                sectionsCustomers = [].concat.apply([], sectionsCustomers);
                return customers.filter(value => sectionsCustomers.includes(value)).length / customers.length
            };
