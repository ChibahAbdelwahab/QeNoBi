    ...
    return {
        ...
        link: function ($scope, element, attrs) {
            ...
            $scope.$on(Parameters.DATASET_EVENTS.DATA_CHANGED, function (event, seriesNum, data, axes) {
                ...
                // Domains
                $scope.xScale.domain(d3.extent(data, function (p) {
                    return p.x;
                }));
                $scope.yScale.domain(d3.extent(data, function (p) {
                    return p.y;
                }));
                $scope.navXScale.domain(d3.extent(data, function (p) {
                    return p.x;
                }));
                $scope.navYScale.domain(d3.extent(data, function (p) {
                    return p.y;
                }));
                ...
                $scope.line = d3.svg.line()
                    .x(function (p) {
                        return $scope.xScale(p.x);
                    })
                    .y(function (p) {
                        return $scope.yScale(p.y);
                    });
                $scope.navLine = d3.svg.line()
                    .x(function (p) {
                        return $scope.navXScale(p.x);
                    })
                    .y(function (p) {
                        return $scope.navYScale(p.y);
                    });
            ...
            $scope.updateScaleOfMatches = function () {
                $scope.svg.selectAll('path.line-match').attr('d',
                    d3.svg.line()
                        .x(function (p) {
                            return $scope.xScale(p.origX);
                        })
                        .y(function (p) {
                            return $scope.yScale(p.origY);
                        })
                );
